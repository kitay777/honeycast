...
const hasUnblurRequest = computed(
    () => !!props.unblur && !!props.unblur.requested
);
const unblurStatus = computed(() =>
    props.unblur && props.unblur.status ? props.unblur.status : null
);
...
const requestUnblur = () => {
    ...
    router.post(
        `/casts/${props.cast.id}/unblur-requests`,
        {},
        {
            onFinish: () => {
...
const startChat = () => {
    ...
    router.post(
        route("casts.startChat", props.cast.id),
        {},
        {
            onFinish: () => {
                startingChat.value = false;
            },
            // コントローラ側 Inertia::location で /chat/{id} にリダイレクトされます
        }
    );
};
</script>
    ...
    <AppLayout>
        ...
        >
            ...
            >
                ...
                <!-- 写真 -->
                <div
                    class="mt-2 relative aspect-[3/2] bg-white rounded overflow-hidden ring-1 ring-black/10"
                >
                    <img
                        :src="
                ...
                <div class="mt-2 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button
                            class="px-4 py-2 rounded bg-[#a99a86] text-black shadow"
                        >
                            指名する
                        </button>
                    </div>
