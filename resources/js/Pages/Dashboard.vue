...
const visibleTextBanners = computed(() =>
    (props.text_banners ?? []).filter(
        (tb) =>
            tb &&
            tb.is_active !== false &&
            String(tb.message ?? "").trim().length > 0
    )
);
const go = (idx) => {
...
const start = () => {
    stop();
    if (adCount.value > 1) timer = setInterval(nextAd, intervalMs);
};
const stop = () => {
    if (timer) {
        clearInterval(timer);
...
<template>
    <AppLayout>
        <div
            class="pt-6 pb-28 px-4 text-white/90 bg-[url('/assets/imgs/back.png')] bg-no-repeat bg-center bg-[length:100%_100%]"
        >
            <!-- =========================
           検索結果（横スクロール）
           ========================= -->
                ...
                >
                    ...
                    >
                        ...
                        <div class="marquee-inner" v-for="rep in 2" :key="rep">
                            <template
                                v-for="(tb, i) in visibleTextBanners"
                                :key="`${rep}-${tb.id}`"
                            >
                                <component
                                    :is="tb.url ? 'a' : 'span'"
                                    :href="tb.url || undefined"
                                    target="_blank"
                                    rel="noopener"
                                ...
                                <span
                                    v-if="i !== visibleTextBanners.length - 1"
                                    aria-hidden="true"
                                    class="opacity-60 px-2"
                                    >|</span
                                >
                            </template>
                        </div>
            ...
            <section v-if="props.ad_banners.length" class="mb-4">
                <div
                    class="relative overflow-hidden rounded-md bg-black/30 h-[400px]"
                    @mouseenter="stop"
                    @mouseleave="start"
                >
...
>
  <div class="ad-scroll">
    ...
    />
                    ...
                    <!-- 左右ボタン -->
                    <button
                        class="hidden md:flex absolute left-2 top-1/2 -translate-y-1/2 z-10 h-9 w-9 items-center justify-center rounded-full bg-black/40 hover:bg-black/60"
                        @click="prevAd"
                        aria-label="prev"
                    >
                        ‹
                    ...
                    <button
                        class="hidden md:flex absolute right-2 top-1/2 -translate-y-1/2 z-10 h-9 w-9 items-center justify-center rounded-full bg-black/40 hover:bg-black/60"
                        @click="nextAd"
                        aria-label="next"
                    >
                        ›
            ...
            <section v-if="props.search_applied" class="mb-8">
                <div
                    class="inline-block px-4 py-1 rounded bg-[#6b4b17] border border-[#d1b05a] text-[18px] tracking-[0.3em]"
                >
                ...
                <div class="relative mt-3">
                    <!-- 左右ボタン（md以上で表示） -->
                    <button
                        class="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-10 h-10 w-10 items-center justify-center rounded-full bg-black/40 hover:bg-black/60"
                        @click="scrollBy(railSearch, -1)"
                    >
                        ‹
                    </button>
                    <button
                        class="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-10 h-10 w-10 items-center justify-center rounded-full bg-black/40 hover:bg-black/60"
                        @click="scrollBy(railSearch, 1)"
                    >
                        ›
                    </button>
                    ...
                    <div
                        ref="railSearch"
                        class="flex gap-4 overflow-x-auto no-scrollbar snap-x snap-mandatory scroll-smooth -mx-2 px-2 py-2"
                    >
                        <div
                ...
                <div class="mt-3">
                    <a
                        href="/dashboard"
                        class="underline text-yellow-200 text-sm"
                        >検索をクリア</a
                    >
                </div>
            </section>
           ...
           ========================= -->
            <template v-else>
                <div v-if="isShopOwner" class="mb-4 text-right">
                    <Link
                        href="/my/shop"
                        class="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-yellow-200 text-black font-semibold shadow hover:brightness-110"
                    >
                        <span>🛍️ マイショップ</span>
                    </Link>
                </div>
                ...
                <section v-if="props.today && props.today.length" class="mb-8">
                    <div
                        class="inline-block px-4 py-1 rounded bg-[#6b4b17] border border-[#d1b05a] text-[18px] tracking-[0.3em] mb-2"
                    >
                        本日 呼べる CAST
                    </div>
                ...
                <!-- 下段：タブボタン -->
                <div
                    class="flex gap-2 justify-between md:justify-start md:gap-3 mb-3"
                >
                    <button
                        v-for="t in tabs"
                        :key="t.key"
                        @click="current = t.key"
                        class="flex-1 md:flex-none px-3 py-2 rounded-md border text-sm md:text-base tracking-widest transition shadow focus:outline-none focus:ring-2 focus:ring-yellow-300"
                        :class="
                            current === t.key
                ...
                <div class="relative">
                    ...
                    <div
                        ref="railTab"
                        class="flex gap-4 overflow-x-auto no-scrollbar snap-x snap-mandatory scroll-smooth -mx-2 px-2 py-2 min-h-40"
                    >
                        <div
                    ...
                    <div
                        ...
                        class="text-center text-white/70 py-10"
                    >
                        該当のキャストがいません。
                    </div>
                </div>
            ...
            <section v-if="props.news.length" class="mb-4">
                <div class="rounded-md bg-black/40 border border-yellow-900/40">
                    <div
                        class="px-4 py-2 text-yellow-200 tracking-[0.3em] border-b border-yellow-900/40"
                    >
                        新着情報
                    </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div class="flex-1">
                                    ...
                                    <p
                                        v-if="n.body"
                                        class="text-sm text-white/80 mt-1 line-clamp-2"
                                    >
                                        {{ n.body }}
                                    </p>
                                </div>
